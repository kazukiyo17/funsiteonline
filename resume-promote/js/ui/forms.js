import {
  getLimits,
  isAllowedUploadFile,
  uploadFileRejectMessage,
  validateOptimizePayload,
} from "../limits.js";
...
export function buildOptimizePayload(form) {
  ...
  if (hasResumeFile && !isAllowedUploadFile(resumeFile)) {
    return { ok: false, message: uploadFileRejectMessage(), formData: fd };
  }
  if (hasJdFile && !isAllowedUploadFile(jdFile)) {
    return { ok: false, message: uploadFileRejectMessage(), formData: fd };
  }
  ...
  if (hasResumeFile) {
    out.append("resume_file", resumeFile);
    out.append("resume_file_name", resumeFile.name);
  }
  if (resumeText) out.append("resume_str", resumeText);
  if (hasJdFile) {
    out.append("jd_file", jdFile);
    out.append("jd_file_name", jdFile.name);
  }
